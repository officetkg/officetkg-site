...
<header class="head">
  <div style="flex:1 1 320px;min-width:0">
    ...
    <p class="sub">PHASE 6 で構築した 3D ジオメトリ。元図面の 2D MASTER 座標をそのまま押し出したもので、画像生成は使っていません。</p>
  </div>
...
<div class="stage">
  ...
  <div class="rail">
    <div class="card">
      <h2>view</h2>
      <div class="views">
        <button class="v" data-az="0"   data-el="89.5">TOP</button>
        <button class="v" data-az="212" data-el="30">SE</button>
        <button class="v" data-az="148" data-el="30">SW</button>
        <button class="v" data-az="328" data-el="30">NE</button>
        <button class="v" data-az="32"  data-el="30">NW</button>
        <button class="v" data-az="180" data-el="4">S</button>
      </div>
      <p class="mono" style="margin:9px 0 0;font-size:10.5px;color:var(--muted)">
        orthographic only — perspective = 0</p>
    </div>
...
<div class="notes">
  <div class="note">
    <h3>座標系</h3>
    <dl class="kv">
      <dt>原点</dt><dd>外壁外面 NW 角</dd>
      <dt>X / Y</dt><dd>東 / 南</dd>
      <dt>単位</dt><dd>1 px = 13.2 mm</dd>
      <dt>外形</dt><dd>365 × 672 px</dd>
      <dt></dt><dd>4818 × 8870 mm</dd>
    </dl>
  </div>
  <div class="note">
    <h3>XY 一致検証</h3>
  ...
  <div class="note">
    <h3>収納壁</h3>
    <p>OPEN / PARKED 固定。東ラン 3 枚は北戸袋、南ラン 2 枚は西戸袋に格納。レールは全長保持、開口は常時クリア。</p>
...
new THREE.GLTFLoader().parse(bytes.buffer,"",gltf=>{
  ...
  root.position.set(-CENTER.x,-CENTER.z,CENTER.y);     // centre after the X rotation
  buildLayers(); setView(212,30); onCut();
...
function setView(az,el){
  const a=az*Math.PI/180, e=el*Math.PI/180, d=RADIUS*2.4;
...
document.querySelectorAll('button.v').forEach(b=>
  b.addEventListener('click',()=>setView(+b.dataset.az,+b.dataset.el)));
