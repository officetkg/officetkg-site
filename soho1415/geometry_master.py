...
# =====================================================================
#  0b. ORIENTATION  --  方位 (参考案により確定)
# =====================================================================
#   KEN 平面図の方位記号 (画素 x1119-1168 / y978-1053) を実測:
#     円     中心 (1143.5, 1012) / 半径 約 22 px
#     上端   y 978-989  (円から 12 px 突出)   <- 長い = 北
#     下端   y 1036-1041 (円から  6 px 突出)
#     "N"    y 1045-1053 -- 下端よりさらに下、記号のキャプション
#   針は上端の方が長く、"N" は記号全体の見出し。
#   参考案の方位記号 (上向き矢印 + 下に "N") が同じ体裁で、この読みを追認した。
#
#   -> 平面の上方向 (+Y) = 真北から時計回り 5.9 deg  =  N6E
#      バルコニー (-Y) は S6W  -- ほぼ真南向き
#      共用廊下側 (+Y) が北、キッチン側 (+X) が東
#
#   ※ 旧版は方位未確定のまま +Y を便宜的に北としていた。
#      実方位は 6 deg しか違わないため、XY 座標の変更は不要。
#      この値はフォトリアル工程の太陽光方向にのみ使用する。
PLAN_UP_BEARING_DEG = 6.0            # 平面 +Y の方位角 (真北 = 0)

def bearing_of(dx, dy):
    """平面ベクトル (dx, dy) の方位角 [deg]."""
    import math
    return (math.degrees(math.atan2(dx, dy)) + PLAN_UP_BEARING_DEG) % 360.0


# =====================================================================
#  0c. FINISH ZONES  --  仕上げ区分 (参考案より / フォトリアル工程用)
# =====================================================================
#   参考案の塗り分けを採取したもの。ジオメトリには影響しない。
FINISH_ZONES = {
    "wood":     ["Living Dining", "Master Bedroom", "廊下"],
    "tile":     ["玄関土間", "Powder Room", "Toilet", "Shower"],
    "concrete": ["Balcony"],
    "shaft":    ["PS"],
}
