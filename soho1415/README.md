...
| 可動収納壁／レール | `MOVABLE_WALL`（**常時開放** — 下記） |
| 窓／サッシ | `WINDOW_SASH` |
| 2人用ワークデスク | `DESK_2P` |
...
| 項目 | Y範囲 | 長さ |
|---|---|---|
| レール全長 | 2410 – 5060 | 2650 |
| 建具走行範囲（閉時に塞がる部分） | 2410 – 4350 | 1940 |
| 戸袋 | 4350 – 5060 | 710 |
| 袖壁（固定） | 5060 – 5330 | 270 |

作図上のパネル継ぎ目が Y 3065 付近・Y 3660 付近にあることから **3枚引込み**
（1枚あたり約647mm）と判断した。戸袋長さ710mmは1枚分に一致する。

**`MOVABLE_WALL_STATE = "OPEN"`（常時開放）**
3枚とも戸袋（Y 4350–5060）に納まり、**Y 2410–4350 の1940mmは素通し**。
Master BedroomとLiving Diningは常時ひと続きの空間となる。
パネルは3レーン（X 1983–2015 / 2023–2055 / 2063–2095）に振り分けて格納。

レール・戸袋・袖壁のXYは開閉状態によらず不変。
参考用に `MOVABLE_WALL_STATE = "CLOSED"` へ切り替えれば閉状態も生成できるが、
...
state: OPEN  | 開口 Y 2410-4350 (1940 mm) は素通し | 戸袋 Y 4350-5060
--- area check ---
